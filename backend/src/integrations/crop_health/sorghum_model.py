...
def _resolve_model_path(configured_path: str) -> Path:
    ...
    raise FileNotFoundError(f"Sorghum model file not found. Checked: {searched}")


class SorghumModelRuntime:
    ...
    def _load(self) -> None:
        ...
        with self._lock:
            ...
            model_path = _resolve_model_path(settings.sorghum_model_path)
            checkpoint = torch.load(model_path, map_location="cpu")
            class_names = list(checkpoint["class_names"])
            img_size = int(checkpoint.get("img_size") or 224)

            model = timm.create_model(
                checkpoint.get("model_name") or "efficientnet_b0",
                pretrained=False,
            ...
            predictions.append(
                {
                    ...
                    "name": display_sorghum_class(class_name),
...
_runtime = SorghumModelRuntime()


def warm_sorghum_model() -> None:
    _runtime._load()
